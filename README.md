...
| **`lda_sents_topic`** | Insert a **topic vector** to be matched with **LDA sentence-level** topics. | Format: "( mag_i * topic_i)''. Example: assuming `d = 100 topics`, the entry `(1, 3, 5, 99)` generates a topic vector `[1, 0, 1, 0, 1, 0, ..., 1, 0]`. The magnitude of vector components can be modified:  `(2*1, -1*3, 3*5, 2*99)` → `[2, 0, -1, 0, 3, 0, ..., 2, 0]`. If no LDA-based matching is needed for sentences, insert `None`. |
