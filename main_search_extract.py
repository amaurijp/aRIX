...
def main():
    ...
    parser.add_argument('-s', '--se_index', default = 'None', help ='Introduzir o index da procura (ver em "SE_inputs.csv") caso a busca seja manual', type = str)
...
def process(revise_search_conditions, se_index):
    ...
    if revise_search_conditions.lower()[0] == 'n' and se_index == 'None':
    ...
    elif revise_search_conditions.lower()[0] == 'n' and int(se_index) in SE_inputs.keys():
        
        #caso a entrada introduzida já tenha sido feita
        if SE_inputs[int(se_index)]['search_status'].lower() == 'finished' and SE_inputs[int(se_index)]['export_status'].lower() == 'finished':
            print(f'\nA rotina de busca e extração para o "se_index" ({int(se_index)}) já está completa.\n')
        
        elif SE_inputs[int(se_index)]['search_status'].lower() != 'finished' or SE_inputs[int(se_index)]['export_status'].lower() != 'finished':
            os.system(f'python {local_dir}/Modules/SEARCH_EXTRACT.py --search_input_index={int(se_index)} --revise_search_cond={revise_search_conditions.lower()} --diretorio={local_dir}')
            print(f'python {local_dir}/Modules/SEARCH_EXTRACT.py --search_input_index={int(se_index)} --revise_search_cond={revise_search_conditions.lower()} --diretorio={local_dir}')
        

    elif revise_search_conditions.lower()[0] == 'y' and int(se_index) in SE_inputs.keys():
        
        #caso a entrada introduzida já tenha sido feita
        if SE_inputs[int(se_index)]['search_status'].lower() == 'finished' and SE_inputs[int(se_index)]['export_status'].lower() == 'finished':
            print(f'\nA rotina de busca e extração para o "se_index" introduzido ({int(se_index)}) já está completa.')
