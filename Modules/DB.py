...
class database(object):
    ...
    def __init__(self, diretorio = None, article_file_type = ''):
        ...
        if os.path.exists(diretorio + '/Outputs/DB_ID.csv'):
            ...
            print('Criando DB...\n')
            self.last_index = 0
            self.DB_ID = pd.DataFrame(columns=['article_ID', 'article_doi', 'Title', 'Addresses', 
                                               'Publication Year', 'Language'], dtype=object)
    ...
    def update(self):
        ...
        for filename in sorted(file_list):
            ...
            if self.article_file_type == 'pdf':
                ...
                if PDF.proc_error_type is None:
                    ...
                    article_address[f'{checked_filename}'] = str( PDF.addresses ) #os paises estão em uma lista
                ...
                if XML.proc_error_type is None:
                    ...
                    article_language[f'{checked_filename}'] = 'English'
                    article_address[f'{checked_filename}'] =  str( XML.addresses ) #os paises estão em uma lista
        ...
        if len(articleID_dict_temp) > 0:
            ...
            counter = 0
            filename_ID_list = list(articleID_dict_temp.items())
            for filename, file_ID in filename_ID_list:
            
            #if len(files_to_add) > 0:

                if file_ID not in self.DB_ID['article_ID'].values:
                
                #for file_N in range(len(files_to_add)):
                    
                    tag_name = get_tag_name( self.last_index + counter)
                    counter += 1
                    ...
                    if self.article_file_type == 'pdf':
                        self.rename_move_save_files(filename, tag_name, file_extension = 'pdf')
                        self.DB_ID.loc[tag_name, 'article_ID'] =  articleID_dict_temp[ filename ]
                        self.DB_ID.loc[tag_name, 'article_doi'] =  article_doi[ filename ]
                        self.DB_ID.loc[tag_name, 'Title'] = article_title[ filename ]
                        self.DB_ID.loc[tag_name, 'Addresses'] = article_address[ filename ]
                        self.DB_ID.loc[tag_name, 'Publication Year'] = int( article_year[ filename ] )
                        self.DB_ID.loc[tag_name, 'Language'] = article_language[ filename ]

                    elif self.article_file_type == 'xml':
                        self.rename_move_save_files(filename, tag_name, file_extension = 'xml')
                        self.DB_ID.loc[tag_name, 'article_ID'] =  articleID_dict_temp[ filename ]
                        self.DB_ID.loc[tag_name, 'article_doi'] =  article_doi[ filename ]
                        self.DB_ID.loc[tag_name, 'Title'] = article_title[ filename ]
                        self.DB_ID.loc[tag_name, 'Addresses'] = article_address[ filename ]
                        ...
                        self.DB_ID.loc[tag_name, 'article_doi'] =  article_doi[ filename ]
                        self.DB_ID.loc[tag_name, 'Title'] = article_title[ filename ]
                        self.DB_ID.loc[tag_name, 'Addresses'] = wbsci_report_filtered.loc[ filename, 'Addresses']
                        self.DB_ID.loc[tag_name, 'Publication Year'] = int( wbsci_report_filtered.loc[ filename, 'Publication Year'] )
                        self.DB_ID.loc[tag_name, 'Language'] = wbsci_report_filtered.loc[ filename, 'Language']
                        
                        #exportando o abstract para txt
                        abstract = wbsci_report_filtered.loc[ filename, 'Abstract']
                        self.export_str_to_file(abstract, tag_name, file_extension = 'txt')
                
                    if counter % 5000 == 0 or (filename == filename_ID_list[-1][0] and file_ID == filename_ID_list[-1][1]):
    ...
    def export_str_to_file(self, text, tag_name, file_extension = ''):
        ...
        if file_extension.lower() == 'txt':
            ...
            if not os.path.exists(self.diretorio + '/DB/' + f'{tag_name}.{file_extension}'):
