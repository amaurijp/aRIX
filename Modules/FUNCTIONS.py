...
def extract_inputs_from_csv(csv_filename = '', diretorio = None, mode = 'search_extract'):
    ...
    if mode.lower() == 'search_extract':
        for line in inputs_DF.index:
            ...
            if inputs_DF.loc[ line , 'lda_search_mode' ].lower() not in ('cosine', 'major_topics'):
                dic[line]['search_inputs']['lda_search_mode'] = None
            else:
                dic[line]['search_inputs']['lda_search_mode'] = inputs_DF.loc[ line , 'lda_search_mode' ]
