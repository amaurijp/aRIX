...
class results(object):
    ...
    def __init__(self, DF_input_name = 'None', diretorio = None):
        ...
        #abrindo dic com os outputs de anova
        if not os.path.exists(self.diretorio + '/Outputs/log/anova_results.json'):
            self.anova_results = dict()
        else:
            self.anova_results = load_dic_from_json(self.diretorio + '/Outputs/log/anova_results.json')
    ...
    def replace_terms_in_DF(self, cats : dict = {'None':'None'}):
    ...
    def plot_cat_barh(self, cat = None, font_scale = None, x_label = None, y_label = None, max_nbars = 10, 
                      plot_margins = None, log_scale = False, terms_to_remove = [], x_label_rotation = 0):
        ...
        cat_val_array = np.array(list(zip(*np.unique(cat_array, return_counts = True))))
        ...
        self.cat_barh(cat_val_array, font_scale = font_scale, x_label = x_label, y_label = y_label, 
                      plot_margins = plot_margins, log_scale = log_scale, plot_counter = plot_counter,
    ...
    def plot_cat_cat_heatmap(self, cats = ['None', 'None'], 
        ...
        if len(cat_cat_val_array) >= 3:
            ...
            self.cat_cat_heatmap(cat_cat_val_array, font_scale = font_scale, mark_scale = mark_scale,
                                x_label = x_label, y_label = y_label, x_label_rotation = x_label_rotation, boxplot_title = boxplot_title,
    ...
    def plot_cat_num_boxplot_anova(self, cat_col = 'None', num_col = 'None', 
                                   stat_func_to_extract = 'avg',
        ...
        for cat in groups_index_dic.keys():
            ...
            if len(vals) >= min_num_vals_occur:
                ...
                anova_cat_list.extend( len(vals) * [cat] )
                anova_val_list.extend( list(vals) )
                anova_median_cat_val[cat] = np.median(vals_cat_dic[ cat ])
        ...
        if len(cats_grouped) >= 3:
            ...
            try:
                self.anova_results[f'boxplot_{cat_col}_{num_col}_{str(cats_terms_to_filter)}']
            
            except KeyError:
                self.anova_results[f'boxplot_{cat_col}_{num_col}_{str(cats_terms_to_filter)}'] = []
                comp = mc.MultiComparison(anova_val_list, anova_cat_list)
                post_hoc_res = comp.tukeyhsd()
                anova_cat_cat_val_array = np.array(post_hoc_res._results_table.data[1:])
                anova_cat_cat_val_array = np.array(list(zip(anova_cat_cat_val_array[: , 0], anova_cat_cat_val_array[: , 1], anova_cat_cat_val_array[: , 3].astype(float) )), dtype = object)
                for cat1, cat2, p in anova_cat_cat_val_array:
                    if p <= 0.05:
                        #salvando o anova em json
                        self.anova_results[f'boxplot_{cat_col}_{num_col}_{str(cats_terms_to_filter)}'].append(
                            f'{cat1} - median = { anova_median_cat_val[cat1] / y_multiplier} : {cat2} - median = { anova_median_cat_val[cat2] / y_multiplier} : p-val = {p}')

                save_dic_to_json(self.diretorio + '/Outputs/log/anova_results.json', self.anova_results)
            ...
            self.box_plot(nums_grouped = nums_grouped, cats_grouped = cats_grouped, n_cats = n_cats, n_points_per_box = n_points_per_box,
                        log_scale = log_scale, y_lims = y_lims, x_label = x_label, y_label = y_label, x_label_rotation = x_label_rotation,
            ...
            #plotando o ANOVA
            #procurando se já existe o setup do plot
            plot_counter = self.get_plot_counter(f'plot_cat_cat_heatmap_anova_{cat_col}_{num_col}_{str(cats_terms_to_filter)}')

            #fazendo o TUKEY HONESTLY SIGNIFICANT DIFFERENCE
            anova_cat_cat_val_filtered_list = []
            for cat1, cat2, p in anova_cat_cat_val_array:
                if cat1 in cats_len_dic.keys() and cat2 in cats_len_dic.keys():
                    anova_cat_cat_val_filtered_list.append((cat1, cat2, p))
            
            anova_cat_cat_val_array = np.array(anova_cat_cat_val_filtered_list, dtype = object)
            self.cat_cat_heatmap(anova_cat_cat_val_array, font_scale = font_scale_anova_grid, mark_scale = mark_scale_anova_grid, box_vals = 0.05, 
                                box_values_font_scale = box_values_font_scale, plot_counter = plot_counter, grid_plot_margins = grid_plot_margins,
                                x_annotation_delta = anova_x_annotation_delta, y_annotation_delta = anova_y_annotation_delta, x_label_rotation = x_label_rotation,
                                bar_label = 'p-val')
        else:
            print('\nErro!\nNão foram encontradas três ou mais categorias para plotar.\n')
    ...
    def plot_cat_cat_num_heatmap(self, cats_col = [], num_col = 'None', 
        ...
        for cat_cat in groups_index_dic.keys():
            ...
            vals = vals[~np.isnan(vals.astype(float))]
            if len(vals) >= min_num_vals_occur:
                vals_cat_dic[ cat_cat ] = vals
                len_cat_list.append( ( cat_cat, len(vals)) )
                anova_cat_list.extend( len(vals) * [cat_cat] )
                anova_val_list.extend( list(vals) )
        
        num_vals_error = []
        cat_cat_val_dic = {}
        cat_cat_stat_func_dic = {}
        for cat_cat in vals_cat_dic.keys():

            if stat_func_to_group == 'avg':
                num_val = vals_cat_dic[ cat_cat ].mean()
            elif stat_func_to_group == 'min':
                num_val = vals_cat_dic[ cat_cat ].min()
            elif stat_func_to_group == 'max':
                num_val = vals_cat_dic[ cat_cat ].max()
            elif stat_func_to_group == 'median':
                num_val = np.median(vals_cat_dic[ cat_cat ])
            
            cat_cat_stat_func_dic[cat_cat] = num_val
            num_c = np.round(num_val, 1) if -10 < num_val < 10 else int(num_val)
            cat_cat_val_dic[cat_cat] = num_c 
            num_vals_error.append((num_val, num_c))


        if len(cat_cat_val_dic) >= 5:
            
            #fazendo o ANOVA
            try: 
                self.anova_results[f'heatmap_{cats_col[0]}_{cats_col[1]}_{num_col}_{str(cats_terms_to_filter)}']
                ...
                for cat_cat1, cat_cat2, p in anova_cat_cat_val_array:
                    if p <= 0.05:
                        #salvando o anova em json
                        self.anova_results[f'heatmap_{cats_col[0]}_{cats_col[1]}_{num_col}_{str(cats_terms_to_filter)}'].append(
            ...
            #caso o número de categorias inserido seja maior que o número de grupos com valores numéricos
            print('  input n_box_size: ', n_box_size)
            n_box_size = len(vals_cat_dic.keys()) if n_box_size > len(vals_cat_dic.keys()) else n_box_size
            print('  output n_box_size: ', n_box_size)

            #sorting dic e coletando só as cats com mais valores
            
            len_cat_list = sorted( len_cat_list, key=lambda x: x[1])
            len_cat_list = sorted( len_cat_list[ - n_box_size : ], key=lambda x: x[0])

            cat_cat_val_array = []
            for cat_cat in cat_cat_val_dic.keys():
                
                cat1 = re.search(r'[\w\s]+(?=_)', cat_cat).group()
                cat2 = re.search(r'(?<=_)[\w\s]+', cat_cat).group()

                if cat_cat in np.array(len_cat_list, dtype = object)[ : , 0]:
                    cat_cat_val_array.append([ cat1, cat2, cat_cat_val_dic[cat_cat]])
            ...
            if len(set(cat_cat_val_array[:, 2])) == 1:
                print('\nERRO!')
    ...
    def plot_scatter_graph(self, num_cols = [], stat_func_to_extract_x = 'avg', stat_func_to_extract_y = 'avg',
        ...
        vals_col1 = self.extract_vals_from_text(indexes = self.results_DF_processed.index, 
                                                num_col = num_cols[0], 
                                                mode = stat_func_to_extract_x, 
                                                axis_multiplier = x_multiplier,
                                                interval_to_filter = x_lims)
        
        vals_col2 = self.extract_vals_from_text(indexes = self.results_DF_processed.index, 
                                                num_col = num_cols[1], 
        ...
        if remove_extreme_vals is not None and remove_extreme_vals < 0.2:
            vals_col1_f = get_array_vals_range(self.results_DF_processed[num_cols[0]].values, remove_extreme_vals)
            vals_col2_f = get_array_vals_range(self.results_DF_processed[num_cols[1]].values, remove_extreme_vals)
        ...
        #procurando se já existe o setup do plot
        plot_counter = self.get_plot_counter(f'plot_scatter_graph_{num_cols[0]}_{num_cols[1]}_{str(cats_terms_to_filter)}')
        ...
        if check_cats_to_plot is True:
            ...
            cat_nums_dic = {}
            anova_median_col_cat_val = {}
            anova_median_col_cat_val[num_cols[0]] = {}
            anova_median_col_cat_val[num_cols[1]] = {}
            anova_num_col_dic = {}
            anova_num_col_dic[num_cols[0]] = {}
            anova_num_col_dic[num_cols[0]]['cats'] = []
            anova_num_col_dic[num_cols[0]]['vals'] = []
            anova_num_col_dic[num_cols[1]] = {}
            anova_num_col_dic[num_cols[1]]['cats'] = []
            anova_num_col_dic[num_cols[1]]['vals'] = []
            ...
            for cat in groups_index_dic:
                ...
                if len(groups_index_dic[cat]) >= min_num_vals_occur:
                    ...
                    anova_num_col_dic[num_cols[0]]['cats'].extend( [cat] * len(df_filtered[num_cols[0]].values) )
                    anova_num_col_dic[num_cols[0]]['vals'].extend( df_filtered[num_cols[0]].values )
                    anova_num_col_dic[num_cols[1]]['cats'].extend( [cat] * len(df_filtered[num_cols[1]].values) )
                    anova_num_col_dic[num_cols[1]]['vals'].extend( df_filtered[num_cols[1]].values )

                    anova_median_col_cat_val[num_cols[0]][cat] = np.median(df_filtered[num_cols[0]].values)
                    anova_median_col_cat_val[num_cols[1]][cat] = np.median(df_filtered[num_cols[1]].values)
            ...
            if len(cat_nums_dic.keys()) >= 2:
                ...
                #fazendo o anova
                comp = mc.MultiComparison(anova_num_col_dic[num_cols[0]]['vals'], anova_num_col_dic[num_cols[0]]['cats'])
                post_hoc_res = comp.tukeyhsd()
                anova_num_col_dic[num_cols[0]]['results'] = np.array(post_hoc_res._results_table.data[1:])
                anova_num_col_dic[num_cols[0]]['results'] = np.array(list(zip(anova_num_col_dic[num_cols[0]]['results'][: , 0], 
                                                                            anova_num_col_dic[num_cols[0]]['results'][: , 1], 
                                                                            anova_num_col_dic[num_cols[0]]['results'][: , 3].astype(float) )), dtype = object)

                comp = mc.MultiComparison(anova_num_col_dic[num_cols[1]]['vals'], anova_num_col_dic[num_cols[1]]['cats'])
                post_hoc_res = comp.tukeyhsd()
                anova_num_col_dic[num_cols[1]]['results'] = np.array(post_hoc_res._results_table.data[1:])
                anova_num_col_dic[num_cols[1]]['results'] = np.array(list(zip(anova_num_col_dic[num_cols[1]]['results'][: , 0], 
                                                                            anova_num_col_dic[num_cols[1]]['results'][: , 1], 
                                                                            anova_num_col_dic[num_cols[1]]['results'][: , 3].astype(float) )), dtype = object)

                cats_cols_to_plot = {}
                median_cat_val = {}
                self.anova_results[f'scattergraph_{num_cols[0]}_{num_cols[1]}_{cats_terms_to_plot[0]}_{str(cats_terms_to_filter)}'] = []
                for num_col in anova_num_col_dic.keys():
                    for cat1, cat2, p in anova_num_col_dic[num_col]['results']:
                        if p <= 0.05:
                            ...
                            median_cat_val[(cat1, cat2)] = {}
                            median_cat_val[(cat1, cat2)][axis] = (anova_median_col_cat_val[num_col][cat1], anova_median_col_cat_val[num_col][cat2])
                            
                            #salvando o anova em json
                            multiplier = x_multiplier if num_col == num_cols[0] else y_multiplier
                            self.anova_results[f'scattergraph_{num_cols[0]}_{num_cols[1]}_{cats_terms_to_plot[0]}_{str(cats_terms_to_filter)}'].append(
                                f'{num_col} > {cat1} - median = { anova_median_col_cat_val[num_col][cat1] / multiplier} : {cat2} - median = { anova_median_col_cat_val[num_col][cat2] / multiplier} : p-val = {p}')
                ...
                for cat1, cat2 in cats_cols_to_plot.keys():
                    ...
                    self.scatter_plot(ixy_all_array = ixy_all_array, cat_nums_dic = cats_cols_to_plot[(cat1, cat2)], 
                                    cats_len_dic = cats_len_dic, axis_median_vals = median_cat_val[(cat1, cat2)],
    ...
    def cat_barh(self, cat_val_array, font_scale = None, x_label = None, y_label = None, 
                 plot_margins = None, plot_counter = None, log_scale = False, x_label_rotation = 0):
    ...
    def cat_cat_heatmap(self, cat_cat_val_array, mark_scale = None, font_scale = None, box_vals = None, box_values_font_scale = None,
                        x_label = None, y_label = None, bar_label = None, x_label_rotation = 90, boxplot_title = '',
    ...
    def box_plot(self, nums_grouped = None, cats_grouped = None, n_cats = None, n_points_per_box = None,
                 log_scale = False, y_lims = None, x_label = None, y_label = None, x_label_rotation = 90,
                 font_scale_boxplot = None, plot_counter = None, boxplot_plot_margins = {}, boxplot_title = ''):
        ...
        ha_aligment = 'right' if x_label_rotation != 90 else 'center'
        ax1.set_xticklabels(cats_grouped, rotation=x_label_rotation, ha=ha_aligment)
    ...
    def scatter_plot(self, ixy_all_array = None, cat_nums_dic = None, cats_len_dic = None, plot_counter = None, 
                     axis_median_vals = None, log_scale_x = False, log_scale_y = False,
                     x_lims = [], y_lims = [], x_label = None, y_label = None, hist_bins_number = 20,
                     set_graph_region = None, boxplot_title = ''):
        ...
        #caso seja dividido por categoria
            ...
            for cat in cat_nums_dic.keys():
                ...
                try: 
                    ax3.axvline(x=axis_median_vals['x'][0], color='black', linestyle='-', linewidth=7, alpha=0.7)
                    ax3.axvline(x=axis_median_vals['x'][1], color='black', linestyle='-', linewidth=7, alpha=0.7)
                except:
                    pass
                ...
                #caso haja diferença estatística entre duas distribuição
                try: 
                    ax2.axhline(y=axis_median_vals['y'][0], color='black', linestyle='-', linewidth=7, alpha=0.7)
                    ax2.axhline(y=axis_median_vals['y'][1], color='black', linestyle='-', linewidth=7, alpha=0.7)
                except:
                    pass
                ...
                ax4.annotate(f'{cat} ({str(cats_len_dic[cat])})', xy=(0, 10 - i), c=palette[i], ha='left', fontsize = 25)
