...
def filter_chemphys_entities(string_text, filename = '', filter_log_dic = None, diretorio = None):
    ...
    pattern_sub_list = [
                        [r'(?<=[\s\(\[])>(?=[0-9])', '> ', 'filtro de >5 -> > 5', True, True ],
                        [r'(?<=[\s\(\[0-9])(weight|w|wt|W)\s*/\s*(weight|w|wt|W)(?=[\s\)\]\,\:\.;])', 'wtperc', 'filtro de % (w / w)', True, True ],
                        [r'(?<=[\s\(\[0-9])(weight|w|wt|W)\s*/\s*(vol|v|V)(?=[\s\)\]\,\:\.;])', 'wtvperc', 'filtro de % (w / v)', True, True ],
                        [r'(?<=[\s\(\[0-9])(vol|v|V)\s*/\s*(vol|v|V)(?=[\s\)\]\,\:\.;])', 'volperc', 'filtro de % (v / v)', True, True ],
                        [r'(?<=[\s\(\[0-9])(ppm)(?=[\s\)\]\,\:\.;])', 'mg l-1', 'filtro de ppm -> mg l-1', True, True ],
                        [r'(?<=[\s\(\[0-9])(mu[;\s]g)(?=[\s\)\]\,\:\.;])', 'ug', 'filtro de mu g -> ug', True, True ],
                        [r'(?<=[\s\(\[0-9])(mu[;\s]m)(?=[\s\)\]\,\:\.;])', 'um', 'filtro de mu m -> um', True, True ],
                        [r'(?<=[\s\(\[0-9])(cc)(?=[\s\)\]\,\:\.;])', 'cm3', 'cc -> cm3', True, True ],
                        [r'(?<=[\s\(\[0-9])(log\(?10\)?([Cc][Ff][Uu]/m[lL]|[Cc][Ff][Uu])?)(?=[\s\)\]\,\:\.;])', 'log', 'filtro de log(10) -> log', True, True ]
                        ]
    ...
    all_PU_combined_dic = []
    for param1, param2 in (("weight", "volume"), ("molarity", "volume")):
    ...
    for u1, u2 in all_PU_combined_dic:
        ...
        mu2 = baseunit2 + '-' + exponent2

        #filtro para padronizar unidades físicas com denominador EX: N/m -> N m-1
        pattern_sub_list = [[r'(?<=[\s\(\[0-9]){u1}\s*/\s*{u2}(?=[\s\)\]\,\:\.;])'.format(u1 = u1 , u2 = u2), 
                             f'{u1} {baseunit2}-{exponent2}', 
                             f'mg/l -> mg l-1', 
                             True, True ]]
        string_text, filter_log_dic = filter_and_check(pattern_sub_list, string_text, filter_log_dic, filename)
    
        #filtro para padronizar unidades físicas com denominador EX: N.m-1 ou Nm-1 -> N m-1
        pattern_sub_list = [[r'(?<=[\s\(\[0-9]){u1}[\.;]*({u2}|{mu2})(?=[\s\)\]\,\:\.;])'.format(u1 = u1 , u2 = u2, mu2 = mu2), 
                             f'{u1} {u2}', 
                             f'mg[.;]l-1 -> mg l-1', 
                             True, True ]]
        string_text, filter_log_dic = filter_and_check(pattern_sub_list, string_text, filter_log_dic, filename)

        #filtro para concentração: mg l(-1) -> mg l(-1)
        pattern_sub_list = [[r'(?<=[\s\(\[0-9]){u1}[\s\.;]*{baseunit2}\(\-{exponent2}\)(?=[\s\)\]\,\:\.;])'.format(u1 = u1 , baseunit2 = baseunit2, exponent2 = exponent2), 
                                f'{u1} {baseunit2}-{exponent2}', 
                                f'mg l(-1) -> mg l-1', 
                                True, True ]]
        string_text, filter_log_dic = filter_and_check(pattern_sub_list, string_text, filter_log_dic, filename)
    ...
    for cat in ('distance', 'log10', 'molarity', 'percentage', 'temperature', 'volume', 'weight'):
        for unit in PU_units[cat]:
            
            #separar números de unidades físicas. EX: 2% -> 2 %         
            pattern_sub_list = [[r'(?<=[0-9]){unit}(?=[\s\)\]\,\:\.;])'.format(unit = unit), 
                                f' {unit}', 
                                f'separando 0mg -> 0 mg', 
                                True, True ]]
            string_text, filter_log_dic = filter_and_check(pattern_sub_list, string_text, filter_log_dic, filename)

            #separar intervalos de números ex: 10-20 mg -> 10 - 20 mg
            pattern_sub_list = [[r'(?<=[0-9]\.?[0-9]*)\-(?=[0-9]\.?[0-9]*\s*{unit}[\s\)\]\,\:\.;])'.format(unit = unit), 
                                f' - ', 
                                f'separando 10-20 mg -> 10 - 20 mg', 
                                True, True ]]
            string_text, filter_log_dic = filter_and_check(pattern_sub_list, string_text, filter_log_dic, filename)
