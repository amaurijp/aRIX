...
class search_engine(object):
    ...
    def set_search_conditions(self, 
        ...
        time.sleep(3)
        #há filtros de seção que usam os topic vectors junto com os word vectors
        if self.bool_search_settings['topic'] is True or self.filter_model is not None:
            
            #carregando as matrizes DOC_TOPIC
            self.topic_matrices = {}

            #lsa_sents
            if self.search_vectors_dic["topic"]["lsa_sents"] is not None or self.filter_model is not None:
                try:
        ...
        if self.filter_model in ('introduction', 'methodology', 'results'):
            ...
            if self.intro_filter.model_found is True:
                self.intro_filter.set_machine_parameters_to_use(tokens_list = tokens_list, wv_type = wv_type, wv_matrix_name = wv_matrix_name, wv_matrix = wv_matrix, doc_topic_matrix = self.topic_matrices['lsa_sents'])
            ...
            if self.methods_filter.model_found is True:
                self.methods_filter.set_machine_parameters_to_use(tokens_list = tokens_list, wv_type = wv_type, wv_matrix_name = wv_matrix_name, wv_matrix = wv_matrix, doc_topic_matrix = self.topic_matrices['lsa_sents'])
            ...
            if self.results_filter.model_found is True:
                self.results_filter.set_machine_parameters_to_use(tokens_list = tokens_list, wv_type = wv_type, wv_matrix_name = wv_matrix_name, wv_matrix = wv_matrix, doc_topic_matrix = self.topic_matrices['lsa_sents'])
                ...
                for index in self.sentDF_copy.index:
                    ...
                    if False not in check_search_bool.values():                        
                        ...
                        if self.nn_filter is True and self.ml_filter is False:
                            ...
                            #checando só uma seção
                            if self.section_to_find == 'introduction' and self.intro_filter.model_found is True:
                                section_proba_result['introduction'] = self.intro_filter.check_sent_in_section_for_NN(index, DF = self.sentDF_copy)
                            elif self.intro_filter.model_found is False:
                                section_proba_result['introduction'] = 0
                            ...
                            #caso seja a seção a ser encontrada e a probabilidade der positiva
                            if section_proba_result[self.section_to_find] >= 0.5:
                                pass
                            #caso não seja a seção
                            else:
                                proba_result = 0

                        #o ML não foi otimizado (*it is not being used*)
    ...
    def find_regex_in_sent(self, text):
        ...
        if len(re.findall( self.search_terms_dic['regex']['regex_pattern'] , text )) > 0:
            #print('- match: regex')
            return True
        else:
            return False
